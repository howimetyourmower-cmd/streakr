      ...
      try {
        ...
        if (snap.exists()) {
          ...
          const roundKey: RoundKey =
            data.currentRoundKey ??
            roundNumberToKey(data.currentRoundNumber ?? 0);

          const cfg: SeasonConfig = {
            season: data.season ?? CURRENT_SEASON,
            currentRoundKey: roundKey,
            currentRoundNumber:
              data.currentRoundNumber ?? roundKeyToNumber(roundKey),
            currentRoundLabel:
          ...
          const defaultCfg: SeasonConfig = {
            ...
            currentRoundLabel:
              ROUND_OPTIONS.find((o) => o.key === defaultKey)?.label ??
              defaultKey,
          };
          ...
          {loading ? (
            ...
            <>
              ...
              <p className="mt-4 text-xs text-slate-500">
                This page writes to Firestore collection <code>config</code>{" "}
                with document ID <code>season-{CURRENT_SEASON}</code>. The Picks
                API reads this value to decide which round of questions to
                serve.
              </p>
            </>
          )}

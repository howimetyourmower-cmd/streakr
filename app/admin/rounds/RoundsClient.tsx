...
export default function RoundsClient() {
  ...
  const handlePublish = async (round: RoundMeta) => {
    try {
      ...
      await updateDoc(doc(db, "rounds", round.id), { published: true });
  ...
  const handleUnpublish = async (round: RoundMeta) => {
    try {
      ...
      await updateDoc(doc(db, "rounds", round.id), { published: false });

      setRounds((prev) => prev.map((r) => (r.id === round.id ? { ...r, published: false } : r)));
    } catch (err) {
  ...
  return (
    <div className="p-6 text-white">
      ...
      <p className="mb-4 text-sm text-gray-300 max-w-2xl">
        Use this page to control which round is live on the Picks page and to set the Sponsor Question for each round.
        <br />
        Clicking <strong>Publish round</strong> will:
        <br />
        1) set the round&apos;s <code>published</code> flag to <code>true</code> (for admin visibility), and
        <br />
                      ...
                      {game.questions && game.questions.length > 0 ? (
                        <div className="space-y-1">
                          {game.questions.map((q, qi) => {
                            ...
                            return (
                              <label key={qi} className="flex items-start gap-2 text-xs text-gray-200">
                                <input
                                  type="radio"

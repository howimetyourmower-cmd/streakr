...
Players should NOT see the raw docId in the UI.

Admin/testing can override via /play/bbl?docId=XXXX

Set this to your current cricketRounds/{docId} to make the public hub work.
*/
...
const overrideDocId = (params.get("docId") || "").trim();
const docId = useMemo(() => {
const picked = overrideDocId || DEFAULT_BBL_DOC_ID;
return (picked || "").trim();
}, [overrideDocId]);

const picksHref = useMemo(() => {
if (!docId) return "";
if (docId === "REPLACE_WITH_CURRENT_BBL_DOC_ID") return "";
...
if (!docId) return "";
if (docId === "REPLACE_WITH_CURRENT_BBL_DOC_ID") return "";
return /api/picks?sport=BBL&docId=${encodeURIComponent(docId)};
}, [docId]);
...
const fadeTimer = window.setTimeout(() => setIsPreloaderFading(true), 3500);
...
if (!apiUrl) {
setError(
"BBL match not set yet. Admin needs to set DEFAULT_BBL_DOC_ID in /app/play/bbl/page.tsx (or open /play/bbl?docId=YOUR_DOC_ID)."
);
setQuestions([]);
return;
}
...
{showPreloader && (
<div
className={fixed inset-0 z-50 flex items-center justify-center bg-black transition-opacity duration-700 ${ isPreloaderFading ? "opacity-0 pointer-events-none" : "opacity-100" }}
>
  ...
  <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 pb-16 pt-8 sm:pt-10">
    ...
    <section className="grid lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] gap-10 items-center mb-14">
      <div>
        ...
        <p className="text-base sm:text-lg text-white/80 max-w-xl mb-6">
          Think you know your cricket? Back your gut ball-by-ball. If you
          miss one, your streak is cooked — back to zip. Nail the clean sweep
          and stack your streak.
        </p>
        ...
        <div className="inline-flex flex-wrap items-center gap-3 mb-6">
          <div className="rounded-full px-4 py-1.5 bg-[#020617] border border-orange-400/70 shadow-[0_0_24px_rgba(255,122,0,0.5)]">
            <span className="text-sm font-semibold text-orange-200">
              Weekly prizes &amp; sponsor gift cards*
            </span>
          </div>
      ...
      <div className="relative">
        <div className="relative w-full h-[260px] sm:h-[320px] lg:h-[360px] rounded-3xl overflow-hidden border border-orange-500/40 shadow-[0_28px_80px_rgba(0,0,0,0.85)] bg-[#020617]">
          <Image
            src="/bbl-hero.jpg"
            alt="Big Bash League under lights"
            fill
    ...
    <section className="mb-10">
      ...
      <p className="text-sm text-white/70 mb-4 max-w-2xl">
        Pick Yes/No questions live. Clean sweep per match. One wrong and it
        resets.
      </p>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-white/10 bg-[#020617] px-4 py-4">
          <p className="text-xs font-semibold text-orange-300 mb-1">
            1 · Pick a cricket question
          </p>
          <p className="text-sm text-white/80">
            Live questions on wickets, runs, boundaries and more. Yes or No
            — back your gut.
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-[#020617] px-4 py-4">
          ...
          <p className="text-sm text-white/80">
            Get every pick in that match right and you add +N. Get one wrong
            and the match resets you to zero.
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-[#020617] px-4 py-4">
          <p className="text-xs font-semibold text-orange-300 mb-1">
          ...
          <p className="text-sm text-white/80">
            Climb ladders, earn badges and talk the talk in the group chat.
          </p>
        </div>
      </div>
    </section>
    ...
    <section className="mb-12">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-white/70">
            A taste of the open questions right now. Jump into Picks to lock
            yours in.
          </p>
        </div>
        ...
        {picksHref ? (
          ...
          <span className="text-sm text-white/40">Awaiting match…</span>
        )}
      </div>
      ...
      {!loading && previewQuestions.length === 0 && !error ? (
        <p className="text-sm text-white/60">
          No open questions right now. Check back closer to first ball.
        </p>
      ) : null}

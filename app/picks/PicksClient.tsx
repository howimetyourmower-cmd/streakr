...
export default function PicksClient() {
  ...
  // ✅ NEW: separate initial load vs refresh to stop UI “disappearing”
  ...
  // ✅ NEW: keep last-good data so refresh never blanks UI
  ...
  const fetchPicks = useCallback(
    async (mode: "initial" | "refresh" = "refresh") => {
      ...
      // only clear err; do NOT clear games/round data (prevents disappearing)
      setErr("");
      ...
      try {
        ...
        // ✅ no ?round => API auto-detects current/next round
        const res = await fetch("/api/picks", { headers: authHeader, cache: "no-store" });
        if (!res.ok) throw new Error(await res.text());

        const data = (await res.json()) as PicksApiResponse;

        const nextRoundNumber = typeof data.roundNumber === "number" ? data.roundNumber : null;
        const nextGames = Array.isArray(data.games) ? data.games : [];

        const nextCurrentStreak = typeof data.currentStreak === "number" ? data.currentStreak : 0;
        const nextLeaderScore = typeof data.leaderScore === "number" ? data.leaderScore : null;
        const nextLeaderName = typeof data.leaderName === "string" ? data.leaderName : null;
        ...
        setLeaderScore(nextLeaderScore);
        ...
        if (lastGoodRef.current) {
          setRoundNumber(lastGoodRef.current.roundNumber);
          setGames(lastGoodRef.current.games);
          setCurrentStreak(lastGoodRef.current.currentStreak);
          setLeaderScore(lastGoodRef.current.leaderScore);
          setLeaderName(lastGoodRef.current.leaderName);
        }
      } finally {
  ...
  const effectiveRoundNumber = roundNumber ?? lastGoodRef.current?.roundNumber ?? null;
  const effectiveGames = games.length ? games : lastGoodRef.current?.games ?? [];

  const roundLabel = effectiveRoundNumber === null ? "" : effectiveRoundNumber === 0 ? "Opening Round" : `Round ${effectiveRoundNumber}`;

  const sortedGames = useMemo(() => {
    return [...effectiveGames].sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());
  }, [effectiveGames]);
  ...
  const gamesPicked = useMemo(() => {
    return effectiveGames.filter((g) => (g.questions || []).some((q) => q.userPick === "yes" || q.userPick === "no")).length;
  }, [effectiveGames]);

  const eligible = gamesPicked > 0;

  const effectiveLeaderScore = leaderScore ?? lastGoodRef.current?.leaderScore ?? null;
  const effectiveLeaderName = leaderName ?? lastGoodRef.current?.leaderName ?? null;
  const effectiveCurrentStreak = (typeof currentStreak === "number" ? currentStreak : 0) ?? lastGoodRef.current?.currentStreak ?? 0;

  const distanceToLeader = useMemo(() => {
    if (effectiveLeaderScore === null) return null;
    return Math.max(0, effectiveLeaderScore - effectiveCurrentStreak);
  }, [effectiveLeaderScore, effectiveCurrentStreak]);

  const leaderProgress = useMemo(() => {
    if (effectiveLeaderScore === null || effectiveLeaderScore <= 0) return 0;
    return clamp((effectiveCurrentStreak / effectiveLeaderScore) * 100, 0, 100);
  }, [effectiveLeaderScore, effectiveCurrentStreak]);
  ...
  const StickyChaseBar = () => {
    ...
    const chaseText =
      effectiveLeaderScore === null ? "Leader loading…" : distanceToLeader === 0 ? "Equal lead" : `Need ${distanceToLeader}`;
                ...
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-white/55 font-semibold truncate">
                      {effectiveLeaderScore === null ? "" : `Leader ${effectiveLeaderScore}${effectiveLeaderName ? ` • ${effectiveLeaderName}` : ""}`}
                    </div>
                  </div>
                </div>
              </div>
  ...
  const DashboardStrip = () => {
    const leaderText =
      effectiveLeaderScore === null ? "Leader loading…" : effectiveLeaderName ? `${effectiveLeaderName} leads` : "Leader";

    const leaderHint =
      effectiveLeaderScore === null
        ...
        <div className="mt-3 grid grid-cols-1 lg:grid-cols-4 gap-2">
          ...
          <div className="rounded-2xl border px-3 py-2" style={cardBase}>
            ...
            <div className="mt-2 flex items-center gap-2">
              <div className="rounded-xl border px-2.5 py-1.5" style={pill}>
                <span className="text-[16px] font-black" style={numStyle}>
                  {effectiveLeaderScore === null ? "—" : effectiveLeaderScore}
                </span>
              </div>
              <div className="min-w-0">
          ...
          <div className="rounded-2xl border px-3 py-2" style={cardBase}>
            ...
            <div className="mt-2 flex items-center gap-2">
              <div className="rounded-xl border px-2.5 py-1.5" style={pill}>
                <span className="text-[16px] font-black" style={numStyle}>
                  {effectiveLeaderScore === null || distanceToLeader === null ? "—" : distanceToLeader}
                </span>
              </div>
              <div className="min-w-0">
                <div className="text-[12px] font-black">{effectiveLeaderScore === null ? "Waiting on data" : "Close the gap"}</div>
                <div className="text-[11px] font-semibold leading-snug" style={{ color: MATCH_HQ.muted }}>
                  Current streak only.
                </div>
              </div>
  ...
  const showSkeleton = loading && (lastGoodRef.current?.games?.length ?? 0) === 0 && games.length === 0;

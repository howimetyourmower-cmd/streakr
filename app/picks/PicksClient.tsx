...
type ApiGame = {
  ...
  sport?: string;      // e.g. "afl", "nrl" etc – optional for now
...
type QuestionRow = {
  ...
  venue: string;
  startTime: string;
  sport: string;       // always set, defaults to "afl"
  quarter: number;
  question: string;
  status: QuestionStatus;
  userPick?: "yes" | "no";
    ...
    const load = async () => {
      try {
        ...
        for (const game of data.games) {
          const sportKey = (game.sport ?? "afl").toLowerCase();
        ...
        {filteredRows.map((row) => {
          ...
          return (
            ...
            >
              ...
              {/* Line 2: date + SPORT BADGE + status (badge sits between date & status) */}
              ...
              {/* Actions row: Yes / No + comments bubble */}
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                ...
                {/* Comments bubble with count (##) style */}
                <button
                  type="button"
                ...
                >
                  <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-slate-600 text-[11px] bg-slate-900/80">
                    💬
                  </span>
                  <span>Comments (##)</span>
                </button>

...
export async function GET() {
  try {
    // First, try to get only the PUBLISHED rounds for the current season
    let roundsSnap = await db
      .collection("rounds")
      .where("season", "==", CURRENT_SEASON)
      .where("published", "==", true)
      .get();

    // If no published rounds yet, fall back to all season rounds (old behaviour)
    if (roundsSnap.empty) {
      roundsSnap = await db
        .collection("rounds")
        .where("season", "==", CURRENT_SEASON)
    ...
    roundsSnap.forEach((roundDoc) => {
      ...
      if (typeof data.round === "number" && roundNumber == null) {

// /app/locker-room/join/JoinRoomClient.tsx
"use client";
...
export default function JoinRoomClient() {
  ...
  const [found, setFound] = useState<FoundRoom | null>(null);
  ...
  const handleJoin = async (e: FormEvent) => {
    ...
    try {
      ...
      const roomsRef = collection(db, "leagues"); // keep Firestore collection as-is
      const qRef = query(roomsRef, where("inviteCode", "==", inviteCode), limit(1));
      ...
      // 2) Check membership using members subdoc (most reliable)
      const memberRef = doc(db, "leagues", room.id, "members", user.uid);
      const memberSnap = await getDoc(memberRef);

      if (memberSnap.exists()) {
        // Already a member — ensure user doc links, then go ladder
        await setDoc(
          doc(db, "users", user.uid),
          { leagueIds: arrayUnion(room.id), updatedAt: serverTimestamp() },
          { merge: true }
        );

        setState("joined");
        router.push(`/locker-room/${room.id}/ladder`);
        return;
      }

      // Re-check room exists
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Pill tone="sky">Locker Room</Pill>
              </div>

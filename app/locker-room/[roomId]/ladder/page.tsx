// /app/locker-room/[roomId]/ladder/page.tsx
"use client";
...
export default function LockerRoomLadderPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        // NOTE: Firestore collection remains "leagues" (renamed in UI/routes only)
        ...
        // Members
        const membersRef = collection(db, "leagues", roomId, "members");
        ...
        if (roomData.managerId && !members.some((m) => m.uid === roomData.managerId)) {
          members.unshift({ uid: roomData.managerId, role: "manager" });
        }
        ...
        const built: Row[] = await Promise.all(
          members.map(async (m) => {
            ...
            const uiRole: "admin" | "member" =
              roomData.managerId && m.uid === roomData.managerId ? "admin" : "member";
    ...
    return (
      <main className="min-h-screen bg-[#050814] text-white">
        <div className="mx-auto w-full max-w-6xl px-4 py-6 md:py-8 space-y-4">
          <Link href="/locker-room" className="text-sm text-sky-400 hover:text-sky-300">
            ← Back to locker room
          </Link>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 min-w-0 flex-wrap">
                    ...
                    {myRow.username && (
                      <span className="text-xs text-white/60 truncate">{myRow.username}</span>
                    )}
